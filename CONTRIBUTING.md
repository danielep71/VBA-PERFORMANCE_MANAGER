...
# 🤝 Contributing to VBA Performance Manager

### Benchmark-grade timing, measurement, and Excel execution control
...
> [!IMPORTANT]
> A benchmark result without sample classification, environment, timing method,
> and baseline is not reproducible evidence. Preserve measurement failures and
> exclusions instead of folding them into successful samples.
...
This is a **source-first measurement library**. The Git diff, not an opaque workbook, is
the review artifact.

| Location | Purpose |
|---|---|
| `src/classes/` | Authoritative performance-manager class source |
| `src/modules/` | Authoritative support modules |
| `test/` | Regression harness |
| `demo/` | Demo builders and usage examples |
| `tools/` | Static analysis and release provenance |
| `docs/` and wiki | Contracts, architecture, and usage guidance |
...
|---|---|
| **Timing session** | Bind a session to one backend and preserve its counter-frequency and rollover semantics. |
| **Measurement validity** | Keep retained, excluded, failed, warm-up, and baseline samples distinguishable; never report a failed measurement as zero duration. |
| **Statistics** | Document sample population, percentile convention, dispersion, baseline subtraction, and units. |
| **Excel state** | Acquire and release application-state ownership transactionally; overlapping managers must not restore state prematurely. |
| **Strict mode** | Preserve the documented distinction between fail-soft status and raised errors. |
| **Performance claims** | Include representative workload, warm-up, timing method, sample size, dispersion, baseline, and environment. |
...
- Run the repository's full regression/certification harness and relevant focused checks.
- Run the hosted static-source gate.
- Exercise changed timing backends, rollover/backwards-clock paths, sample classification, strict-mode behavior, and cleanup.
- For Excel-state changes, test overlapping instances, partial failure, and restoration on success and error.
- Record exact commit, Excel/Windows/Office bitness, backend, timer-resolution state, verdict, skips, and cleanup.

Treat cleanup and session-release failures as test failures. Validate statistics against an independent calculation where formulas or percentile behavior change, and test both ordinary and degenerate sample sets.

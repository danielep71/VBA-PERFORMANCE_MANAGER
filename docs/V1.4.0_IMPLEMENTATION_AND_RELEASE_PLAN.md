...
> **REVISION 12 — 30 August 2026**
>
> **Supersedes:** Revision 11.
>
> **Revision 12 correction:** #28's implementation (`1c1f794`), exact strict
> error assertion (`2c4f821`) and wiki (`3f89a02`) are delivered, with 79
> cases, 623 assertions and zero failures plus 12 passing static checks. The
> closure audit found two remaining criteria: add a non-strict all-endpoint-
> failed overhead regression, and replace the root README's stale statement
> that #28 remains outstanding. #28 therefore stays open.
>
...
> **Status:** Phases 0–3A delivered; Phase 4 implementation delivered, closure
> blocked by one regression subcase and the root README reconciliation
>
...
| Live issue snapshot | #28 closure audit refreshed `2026-08-30` |
| Open portfolio distribution | **19 total** · v1.4.0: **3** · v1.4.1: **9** · v1.5.0: **7** |
| Open-issue assignment | All 19 assigned to `danielep71` |
| Audit disposition | #28 remains open with two explicit closure blockers; title, labels, milestone and assignment are correct |
| v1.4.0 milestone | GitHub milestone `#3` |
| Open v1.4.0 issues | **3**: #24, #28 and #29 |
| Closed v1.4.0 issues | **5**: #2, #23, #25, #26 and #27 |
...
| Delivered implementation head | `2c4f821ce90e38522e0ea52c83129b65743bce91` |
| Delivered so far | #26 (`6d44f2f`, `9362b8f`, `9064cff`) · #25 (`37c64fc`, manual dispatch PASS) · #23/#27 (`e5031fc`, `e7ab62d`) · #2 (`eff392f`, `e8eae90`) · #28 implementation (`1c1f794`, `2c4f821`) with closure blockers remaining |
| Current Excel regression | `2c4f821` · 79 cases · 623 assertions · 0 failures · Microsoft 365 Excel 64-bit Version 2607 Build 16.0.20228.20188 |
...
| Fault injection | Workload start/endpoint seams landed with #23 and now reach overhead sampling in `1c1f794`; #28 still needs its total endpoint-failure regression and README reconciliation before closure. |
...
- [ ] Forced QPC start failure is covered in strict and non-strict modes with `AlignToNextTick = True`.
...
| 5 | [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) — Align MeasureOverhead_Samples fault injection and failure/rejection evidence | `bug` `P3` `measurement` `api` `testing` | **Open, implementation delivered** — `1c1f794`, `2c4f821`, wiki `3f89a02`; total-failure regression and README remain |
| 6 | [#24](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/24) — Route OverheadMeasurement_Seconds through the validated sample path | `bug` `P2` `measurement` `api` | #23 delivered; waits for #28's two closure blockers |
...
| Behavior/API change | Required same-commit `[Unreleased]` entry | Revision 12 status |
|---|---|---|
...
## Phase 4 — [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28): Align MeasureOverhead_Samples fault injection and failure/rejection evidence — implementation delivered, closure pending

### Delivered in `1c1f794` and `2c4f821`
...
7. Clean, partial endpoint-failure, partial rejection, all-rejection, warm-up exclusion, strict worker-raise, recovery and legacy-call-shape regressions pass.
...
1. Add the missing non-strict **all-endpoint-failed** overhead subcase. It must assert `ERR_CPM_MEASURE_NO_VALID_SAMPLES`, `FailedReadsOut = Iterations`, `RejectedSamplesOut = 0`, the native failure status, publication before raise and seam recovery.
2. Reconcile the root README. It still says `MeasureOverhead_Samples` exposes no evidence and #28 “remains outstanding”; replace that text with the three-output contract, count equation, warm-up asymmetry, `-1` sentinel, backend-homogeneity rule and aggregate-raise behavior.
3. Rerun Excel and all 12 static checks against the resulting exact SHA before closure.
...
The class header and wiki now carry this equation. The root README remains the
last stale documentation surface and must be corrected before #28 closes.

**Exit gate:** not yet complete. The implementation, changelog and wiki gates
pass, but the required all-endpoint-failed regression and README reconciliation
remain. Do not close #28 or begin #24 until both land and the exact-SHA Excel and
static gates are repeated.
...
| `feat(measurement): expose overhead failure and rejection evidence` | #28 overhead three-output evidence, separate classification, cleanup, regressions and same-commit `Added`/`Changed` entries | **Partial** — `1c1f794`, `2c4f821`, wiki `3f89a02`; total-failure regression and README remain |
| `fix(measurement): validate overhead means and text` | #24 authoritative path, numeric raise, undefined text and same-commit behavioral-correction entry | Pending after #28 closes |
...
| Issue | Minimum closure evidence | State at Revision 12 |
|---|---|---|
...
| [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) | Separate seams, three overhead outputs, separate count semantics, clean/partial/total/rejection regressions, worker-raise boundary, cleanup proof, README/wiki/header agreement and distinct same-commit `Added`/`Changed` entries | **Open** — implementation at `1c1f794`/`2c4f821` and wiki `3f89a02`; all-endpoint-failed regression and README remain |
...
## Appendix A — Revision 12 live open-issue reconciliation

The 30 August 2026 audit re-read the open portfolio against the current branch,
documentation, repository configuration and milestone intent. After #2 closed,
19 issues remain open and correctly scheduled: three in v1.4.0, nine in v1.4.1
and seven in v1.5.0. All 19 are assigned to `danielep71`.
...
- #24 now carries `api`, records #23 as delivered and names #28 as the remaining
  implementation prerequisite.
- #28 records the overhead implementation delivered in `1c1f794`, the exact
  strict-error assertion in `2c4f821`, the 79-case/623-assertion zero-failure
  Excel result and wiki `3f89a02`. It remains open because the required
  all-endpoint-failed overhead regression is absent and the root README still
  describes #28 as outstanding.
...
| [#24](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/24) — Route OverheadMeasurement_Seconds through the validated sample path | v1.4.0 | `bug` `P2` `measurement` `api` | Valid; waits for #28's regression and README closure work |
| [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) — Align MeasureOverhead_Samples fault injection and failure/rejection evidence | v1.4.0 | `bug` `P3` `measurement` `api` `testing` | Valid; implementation/wiki delivered, total-failure regression and README remain |
...
snapshot below predates `eff392f` and the Revision 12 verification; current
...
> Revision 12's milestone register, Appendix A and live GitHub records are

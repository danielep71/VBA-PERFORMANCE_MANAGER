...
> **REVISION 10 — 30 August 2026**
>
> **Supersedes:** Revision 9.
>
> **Revision 10 correction:** #2's aligned strict/non-strict coverage landed in
> `eff392f` with 78 cases, 592 assertions and zero failures. Closure remains
> blocked by one exactness gap: the aligned strict subcase accepts any non-zero
> error instead of asserting `ERR_CPM_QPC_READ_FAILED`. The issue body and audit
> comment now state that final test-only requirement explicitly.
>
...
> **Status:** Phases 0–3 delivered; add #2's exact strict-error assertion and
> rerun its exact-SHA gates before beginning Phase 4
>
...
| Live issue snapshot | #2 closure verification refreshed `2026-08-30` |
| Open portfolio distribution | **20 total** · v1.4.0: **4** · v1.4.1: **9** · v1.5.0: **7** |
| Open-issue assignment | All 20 assigned to `danielep71` |
| Audit disposition | All remain valid; no closure or milestone move required |
| v1.4.0 milestone | GitHub milestone `#3` |
| Open v1.4.0 issues | **4**: #2, #24, #28 and #29 |
| Closed v1.4.0 issues | **4**: #23, #25, #26 and #27 |
...
| Delivered implementation head | `eff392f8c5294a29b06097f1b46ebc4a9a5c7804` |
| Delivered so far | #26 (`6d44f2f`, `9362b8f`, `9064cff`) · #25 (`37c64fc`, manual dispatch PASS) · #23/#27 (`e5031fc`, `e7ab62d`) · #2 partial (`eff392f`) |
| Current Excel regression | `eff392f` · 78 cases · 592 assertions · 0 failures · Microsoft 365 Excel 64-bit Version 2607 Build 16.0.20228.20188 |
| Current static result | 12 checks · 0 failures · passed |
| Wiki evidence for #23/#27 | `052f903c96b207184de898d86efeed51ba1d5b6f` · `f5ee7912ceb93d05e96e3f24d4f77f4c16cece88` |
...
| Decision | Resolution |
|---|---|
| [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) | **Aligned coverage landed; exact-error assertion remains.** `eff392f` extends both existing cases with `AlignToNextTick = True`, but the strict aligned subcase must assert `ERR_CPM_QPC_READ_FAILED` rather than accepting any non-zero error. No production change is expected. |
...
| Fault injection | Workload start/endpoint seams landed with #23; #28 extends the same explicit model to overhead sampling. |
...
| 4 | [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) — Failed QPC start read commits a zero-based session in non-strict mode | `bug` `P1` `timing` `testing` | **Reopened** — runtime fixed; aligned strict/non-strict regression remains |
| 5 | [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) — Align MeasureOverhead_Samples fault injection and failure/rejection evidence | `bug` `P3` `measurement` `api` `testing` | Workload seams delivered by #23; overhead extension remains |
| 6 | [#24](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/24) — Route OverheadMeasurement_Seconds through the validated sample path | `bug` `P2` `measurement` `api` | #23 delivered; #28 remains the implementation prerequisite |
...
| Behavior/API change | Required same-commit `[Unreleased]` entry | Revision 10 status |
|---|---|---|
...
| #28 overhead evidence | Separate `Added` entry naming `MeasureOverhead_Samples` `FailedReadsOut`, `LastFailureStatusOut` and trailing `RejectedSamplesOut`; mention split deterministic seams where useful | Pending |
...
## Phase 3A — [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2): complete aligned QPC start-failure regression coverage — open, one assertion remains
...
- The scope boundary names the later loop-read and timeout paths tracked in
  #44.
- The recorded run is 78 cases, 592 assertions and zero failures on Microsoft
  365 Excel 64-bit Version 2607 Build 16.0.20228.20188, with all 12 static
  checks passing.

### Remaining test change

The aligned strict subcase currently converts any non-zero `Err.Number` into a
Boolean `Raised` flag and clears the error. Capture the number before clearing
it and assert exact equality with `ERR_CPM_QPC_READ_FAILED`
(`vbObjectError + 1007`). Then rerun Excel and all static checks against the
resulting exact SHA and record the new deterministic assertion total.
...
**Exit gate:** the aligned strict path asserts the exact error number; both
aligned modes pass in real 64-bit Excel at the resulting exact test commit; all
12 static checks pass; #2 closes with links to the original runtime fix,
`eff392f`, the exact-error follow-up and the final execution evidence.

## Phase 4 — [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28): Align MeasureOverhead_Samples fault injection and failure/rejection evidence
...
| `test(timing): cover aligned QPC start-failure paths` | #2 extends the existing strict/non-strict cases with `AlignToNextTick = True`; no production change expected | **Partial** — `eff392f`; exact strict-error assertion and fresh Excel evidence remain |
| `test(measurement): split worker fault injection` | #28 start/endpoint seams, overhead three-output evidence, separate count semantics, cleanup tests and same-commit `Added` entry | Pending after #2 |
| `fix(measurement): validate overhead means and text` | #24 authoritative path, numeric raise, undefined text and same-commit behavioral-correction entry | Pending after #28 |
...
| [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) | Separate seams, three overhead outputs, separate count semantics, worker-raise boundary, cleanup proof and distinct same-commit `Added` entry | Open |
...
## Appendix A — Revision 10 live open-issue reconciliation

The 30 August 2026 audit re-read all 20 open issues against the current branch,
documentation, repository configuration and milestone intent. All remain
genuinely open and correctly scheduled. The live distribution is four issues
in v1.4.0, nine in v1.4.1 and seven in v1.5.0. All are assigned to
`danielep71`.

### Corrections applied directly on GitHub

- #2 now records the aligned coverage delivered in `eff392f`, the verified
  78-case/592-assertion Excel result and the remaining exact strict-error
  assertion. A non-closing audit comment prevents premature closure.
- #24 now carries `api`, records #23 as delivered and names #28 as the remaining
  implementation prerequisite.
- #28 distinguishes the workload-side seams already delivered by #23 from the
  still-open `MeasureOverhead_Samples` API, evidence and cleanup work. Criteria
  already proved by #23 are checked without overstating overhead completion.
...
- #44 now owns deterministic alignment-loop read-failure and timeout coverage
  ...
  #2 or v1.4.0.
- #11 and #24–#44 are assigned to `danielep71`; #2 was already assigned.

### Full open-issue register after audit

| Issue | Milestone | Labels | Audit disposition |
|---|---|---|---|
| [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) — Failed QPC start read commits a zero-based session in non-strict mode | v1.4.0 | `bug` `P1` `timing` `testing` | Valid; aligned coverage landed in `eff392f`, exact strict-error assertion and fresh evidence remain |
...
| [#24](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/24) — Route OverheadMeasurement_Seconds through the validated sample path | v1.4.0 | `bug` `P2` `measurement` `api` | Valid; waits for #28 |
| [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) — Align MeasureOverhead_Samples fault injection and failure/rejection evidence | v1.4.0 | `bug` `P3` `measurement` `api` `testing` | Valid; partially delivered through #23, overhead scope remains |
...
snapshot below predates `eff392f` and the Revision 10 verification; current
...
- **Assignee:** `danielep71`
- **Last updated:** `2026-08-30T13:49:34Z`
...
> Revision 10's milestone register, Appendix A and live GitHub records are

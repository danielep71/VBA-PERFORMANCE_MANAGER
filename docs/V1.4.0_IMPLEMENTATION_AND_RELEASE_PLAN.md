...
> **REVISION 17 — 30 August 2026**
>
> **Supersedes:** Revision 16.
>
...
>
> **Scope remains:** #11 and #44 are deferred to v1.4.1. #29 remains in v1.4.0
> and closes from controlled manual exact-SHA 32-bit evidence using the Phase 0
> JSON schema.

> **Status:** Phases 0–6 complete and signed off; #29 deferred to v1.4.1; merge → 64-bit exact-SHA certification → tag → provenance remain
>
...
| Live issue snapshot | #24 closure verified `2026-08-30` |
| Open portfolio distribution | **17 total** · v1.4.0: **1** · v1.4.1: **9** · v1.5.0: **7** |
| Open-issue assignment | All 17 assigned to `danielep71` |
| Audit disposition | #24 closed as completed; title, body, labels, milestone, assignment and comments are correct |
| v1.4.0 milestone | GitHub milestone `#3` |
| Open v1.4.0 issues | **1**: #29 |
| Closed v1.4.0 issues | **7**: #2, #23, #24, #25, #26, #27 and #28 |
| Explicitly deferred | #11 and #44 → v1.4.1 milestone `#4` |
...
> complete aligned QPC start-failure regression coverage,
...
- [ ] All eight issues assigned to the v1.4.0 milestone satisfy every acceptance criterion and are closed with evidence.
...
| 7 | [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) — Publish Office 32-bit execution certification | `enhancement` `release` `P2` `testing` | Schema/manual route frozen; final exact-SHA evidence pending; not #11 |
...
| Behavior/API change | Required same-commit `[Unreleased]` entry | Revision 16 status |
|---|---|---|
...
## Release-process exception: closing #29 without invalidating it

Closing #29 requires 32-bit evidence bound to the exact release SHA. Recording
that closure in this plan would change the SHA the evidence describes, and
re-certifying would require another plan edit. That loop does not terminate.

The locked sequence breaks it by deferring the plan update past the tag:

1. Finalize this plan with #29 **pending external exact-SHA certification**.
2. Merge `release/v1.4.0` into `main`.
3. Record the resulting `main` SHA and certify it on 64-bit and 32-bit.
4. Close #29 with the 32-bit evidence. No repository commit accompanies this.
5. Create and push annotated `v1.4.0` on that unchanged certified SHA, verify
   `v1.4.0^{commit}` resolves to the recorded SHA, then generate and verify
   provenance.
6. Push a post-release plan revision recording #29 closure, the tag and the
   provenance. `main` then moves ahead of the immutable release tag, which is
   correct: the tag is the release, not the branch head.

The certification therefore never describes a tree that a later plan edit
changed, and the plan still records the outcome within the same release task.
...
| Issue | Minimum closure evidence | State at Revision 16 |
|---|---|---|
...
| [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) | Completed exact-SHA real 32-bit JSON artifact with zero failures and explicit Win32 path coverage | Open |
...
## Appendix A — Revision 16 live open-issue reconciliation

The 30 August 2026 audit re-read the open portfolio against the current branch,
documentation, repository configuration and milestone intent. After #24 closed,
17 issues remain open and correctly scheduled: one in v1.4.0, nine in v1.4.1
and seven in v1.5.0. All 17 are assigned to `danielep71`.
...
- #28 is closed after `cdf2e4d` supplied the missing all-endpoint-failed
  ...
  pass independently.
- #29 records that the standalone schema and controlled manual route are frozen
  while making clear that no real 32-bit release-candidate artifact exists yet.
...
| [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) — Publish Office 32-bit execution certification | v1.4.0 | `enhancement` `release` `P2` `testing` | Valid; schema/manual route frozen, certification artifact pending |

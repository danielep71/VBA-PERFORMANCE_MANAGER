...
> **REVISION 9 — 30 August 2026**
>
> **Supersedes:** Revision 8.
>
> **Revision 9 correction:** #44 records the alignment-loop read-failure and
> spin-guard timeout coverage deliberately excluded from #2. It belongs to
> v1.4.1, does not block v1.4.0 and leaves the current execution order
> unchanged. The live backlog now contains 20 assigned issues.
>
...
> **Status:** Phases 0–3 delivered; close the #2 aligned-start regression gap
...
| Live issue snapshot | Full open-backlog audit completed `2026-08-30` |
...
| Delivered implementation head | `e7ab62de4b411204a7e527a855eea30c6aea51f0` |
| Delivered so far | #26 (`6d44f2f`, `9362b8f`, `9064cff`) · #25 (`37c64fc`, manual dispatch PASS) · #23/#27 (`e5031fc`, `e7ab62d`) |
| Current Excel regression | 78 cases · 580 assertions · 0 failures · Microsoft 365 Excel 64-bit Version 2607 Build 16.0.20228.20188 |
...
| Decision | Resolution |
|---|---|
| [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) | **Reopened for one missing regression only.** Extend the existing strict and non-strict QPC start-failure cases with `AlignToNextTick = True`; do not rewrite the proven v1.2.0 runtime fix unless the test exposes a defect. |
...
| Behavior/API change | Required same-commit `[Unreleased]` entry | Revision 9 status |
|---|---|---|
...
## Phase 3A — [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2): complete aligned QPC start-failure regression coverage — open

The transactional production correction remains the v1.2.0 implementation in
`Start_TryCapture`: a failed requested capture either raises without committing
in strict mode or captures method 2 before committing in non-strict mode. The
closure audit found only an evidence gap. Existing tests force the direct QPC
start path, and ordinary tests exercise aligned starts, but no test combines
forced QPC start failure with `AlignToNextTick = True`.

### Required test change

Extend the two existing cases rather than adding parallel cases:

1. In `Test_FaultInject_QpcStartFailure_Strict`, protect a known-good session,
   arm the one-shot raw QPC failure and call
   `StartTimer cPM_MethodQPC, True, <label>`.
2. Assert `ERR_CPM_QPC_READ_FAILED` (`vbObjectError + 1007`),
   `cPM_ReadQpcFailed`, and preservation of the prior session, backend, start
   timestamp and run label.
3. Prove the one-shot seam self-clears with a following successful QPC start.
4. In `Test_FaultInject_QpcStartFailure_NonStrict`, arm the same seam and call
   `StartTimer cPM_MethodQPC, True`.
5. Assert a committed method-2 session, `cPM_ReadFallbackToMethod2`, a positive
   non-sentinel start timestamp and a sane short elapsed interval.
6. Prove a following QPC start is unaffected.

Keeping the coverage inside the two existing strict/non-strict cases leaves
`TotalSteps = 78`. Update the deterministic assertion total in the test output
and commit evidence after the Excel run; do not predict it in this plan.
...
**Exit gate:** both aligned strict and aligned non-strict forced-failure paths
pass in real 64-bit Excel at the exact test commit; all 12 static checks pass;
#2 closes with links to the original runtime fix and the new aligned evidence.
...
| `test(timing): cover aligned QPC start-failure paths` | #2 extends the existing strict/non-strict cases with `AlignToNextTick = True`; no production change expected | **Next** |
...
| Issue | Minimum closure evidence | State at Revision 9 |
|---|---|---|
| [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) | Existing runtime correction plus forced aligned QPC start failure in strict/non-strict modes, transactional preservation/fallback and one-shot recovery | **Open** — aligned regression missing |
...
## Appendix A — Revision 9 live open-issue reconciliation
...
|---|---|---|---|
| [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) — Failed QPC start read commits a zero-based session in non-strict mode | v1.4.0 | `bug` `P1` `timing` `testing` | Valid; reopened only for the missing aligned forced-failure regression |
...
### Revision 7 closure delta retained

Revision 6 reproduced #24–#29. Revision 7 added the complete current record
for #2 because the closure audit moved it into the v1.4.0 milestone.
...
> Revision 9's milestone register, Appendix A and live GitHub records are

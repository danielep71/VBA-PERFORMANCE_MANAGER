...
> **REVISION 7 — 30 August 2026**
>
> **Supersedes:** Revision 6.
>
> **Revision 7 correction:** the milestone and evidence snapshot now reflects
> what has actually landed. #23 and #27 are closed with repository, Excel and
> wiki evidence. A closure audit reopened #2 because its v1.2.0 runtime fix was
> never forced through the `AlignToNextTick = True` QPC start path. The missing
> aligned strict/non-strict regression is now an explicit v1.4.0 gate; the
> production correction itself remains valid and unchanged.
>
...
| Live issue snapshot | Closure audit completed `2026-08-30` |
...
| 5 | [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) — Align MeasureOverhead_Samples fault injection and failure/rejection evidence | `bug` `P3` `measurement` `api` `testing` | #23 contract and split seams |
| 6 | [#24](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/24) — Route OverheadMeasurement_Seconds through the validated sample path | `bug` `P2` `measurement` | #23 and #28 |
| 7 | [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) — Publish Office 32-bit execution certification | `enhancement` `release` `P2` `testing` | Final exact-SHA candidate; not #11 |
...
| Behavior/API change | Required same-commit `[Unreleased]` entry | Revision 7 status |
|---|---|---|
...
| Issue | Minimum closure evidence | State at Revision 7 |
|---|---|---|
...
## Appendix A — Revision 7 live issue delta

Revision 6 already reproduced #24–#29. Revision 7 adds the complete current
record for #2 because the closure audit moved it into the v1.4.0 milestone.
...
> Revision 7 milestone register and live GitHub records are authoritative.
> During the 30 August audit, the complete bodies for the still-open #24, #28
> and #29 were rechecked and remained current. Their live titles, labels and
> timestamps are reflected here; the #23/#25/#26/#27 entries remain frozen
> historical records.

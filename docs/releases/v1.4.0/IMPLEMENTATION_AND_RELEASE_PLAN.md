...
> **REVISION 23 — 31 August 2026**
>
> **Supersedes:** Revision 22.
>
> **Record status:** Archived final v1.4.0 implementation and release record.
> Future edits are limited to corrections of the historical v1.4.0 record.
>
> **Revision 23 archive relocation:** after publication, certification and the
> closed-milestone audit, the active plan no longer belongs at the root of
> `/docs`. The complete record now lives under
> `docs/releases/v1.4.0/IMPLEMENTATION_AND_RELEASE_PLAN.md`, alongside the
> release-document index. The former path is retained as a redirect notice so
> existing issue comments and external links do not fail.
>
> This is a documentation-only relocation. No issue, source, workflow, release
> asset, tag or evidence claim changed.
>
> **Revision 22 closed-milestone audit:** all seven closed v1.4.0 issues
> (#2, #23–#28) were re-fetched with their complete bodies and comment threads
> and checked against the tagged source, released wiki history and retained
> evidence. Titles, labels, assignment, milestone, `completed` closure reasons
> and substantive checkbox states are correct. #26 retains its one transparent
> commit-atomicity process exception; no deliverable is missing.
>
> One evidence-record correction was required. #27's original closing comment
> said its wiki baseline-subtraction guidance was already complete, but
> `Benchmarking-Guidance.md` still described #27 as open and the three outputs
> as absent. Wiki commit `3a76948` repaired that missed guidance during #24. The
> #27 body now names the later correction and its thread records the chronology;
> its API, regression evidence and completed closure state are unchanged.
>
> The `v1.4.0` tag independently passes all 12 static checks, `git diff --check`
> is clean, every repository implementation commit cited by the seven issues is
> an ancestor of the tag, and every cited wiki commit is retained in the final
> wiki history. This revision changes issue records and this plan only.
>
> **Revision 21 v1.4.1 assurance audit:** #36 is corrected to describe the
> delivered #27/#28 measurement-evidence APIs. Its remaining five
> contradictions stay narrow; compile-safety and label-workflow work are not
> folded into it.
>
> #45 adds two fixture-backed compile-safety checks for same-procedure
> `On Error GoTo` labels and assigned-but-undeclared identifiers. #46 combines
> complete missing-label aggregation with the `actions/github-script@v8`
> runtime upgrade. Both are assigned to `danielep71` in v1.4.1.
>
> This revision changes issue records and this plan only. No workflow, linter or
> production source was modified.
>
> **Revision 20 post-release reconciliation:** v1.4.0 is published. The README's
> execution-evidence section carried a `Pending` final-certification block and a
> development-line table, both written before the tag existed. Those are replaced
> by the published v1.4.0 record, with v1.3.0 demoted to previous.
>
> The published assets were downloaded from the release and re-hashed. The
> workbook is `5eddd647…` and the manifest is `d13567b4…`, matching the manifest
> and the local copies, so the upload preserved bytes and a downloader can verify
> what they received. That path had never been tested.
>
> One earlier finding is withdrawn. The README recorded the published workbook as
> `PERFORMANCE.MANAGER.xlsm` while the local build is `PERFORMANCE MANAGER.xlsm`,
> and this was called a documentation defect. It was not: GitHub replaces the
> space with a dot on upload, so both release pages show the dotted name and the
> README was correct. A note now records the difference rather than a correction.
>
> **Revision 18 issue-state correction:** #29 was closed as `completed`
> without a real Office 32-bit execution artifact or closing comment. That
> closure did not satisfy its evidence gate. #29 is therefore reopened, moved
> to v1.4.1 and made explicitly non-blocking for v1.4.0.
>
> The issue keeps its title, labels and assignment. Its body now permits
> evidence from any controlled real 32-bit Office host, including a credible
> contributor-provided environment; no evidence may be inferred from source
> inspection or a 64-bit run.
>
> The annotated `v1.4.0` tag resolves to
> `a5390b4c6ca56ebbd87eca121b5167ee5dc09963`, and the retained 64-bit execution
> and static artifacts both identify that same commit on `main`. The exact-tag
> evidence rule is satisfied.
>
> Revision 18 described a `5a4d57b` artifact as the retained evidence. That pair
> was the pre-documentation run, superseded when the README, plan, `.gitignore`
> and untracking commits moved `main` past it. The `a5390b4` pair replaced it and
> is the release evidence.
>
> Both pairs carried the same filenames, which is how the wrong one came to be
> cited. Certification artifacts are henceforth filed one directory per certified
> SHA, so a superseded run cannot be mistaken for a current one.
>
> **Revision 17 scope decision:** #29 moves to v1.4.1 and no longer blocks this
> release.
>
> Certifying 32-bit requires a 32-bit Office host, which cannot coexist with
> 64-bit Office on one Windows installation. No such host is available, so the
> choice was between holding the release on unavailable hardware, narrowing the
> supported set, or shipping with the gap stated.
>
> v1.4.0 ships executed and certified on Microsoft 365 Excel 64-bit. The source
> retains its Office 32-bit compatibility branches, so 32-bit behavior is
> **unverified, not unsupported**. No changelog entry accompanies this: support
> is neither changed nor removed, and only its verification is deferred.
>
> The delivered measurement corrections are unaffected. Seven issues closed on a
> certified SHA, with one assurance gap named rather than glossed.
>
> **Revision 16 correction:** three defects in Revision 15 itself.
>
> First, Revision 15 recorded that `eb1e77a` corrected **five** stale wiki
> claims. A sixth remained: the error table in `Statistics and Measurement`
> described `ERR_CPM_MEASURE_NO_VALID_SAMPLES` as "No valid measured endpoint
> read", which excludes the all-rejected case where every read succeeded on the
> wrong backend. Corrected in `9dffa3f`.
>
> Second, the status line listed "exact-SHA certification, merge, tag" —
> reversing the locked release order. Certification must follow the merge,
> because a merge commit produces a SHA that no pre-merge artifact describes.
>
> Third, three headings still referenced Revision 14. Renumbered.
>
> **Revision 15 correction:** Phase 6 is complete on both repositories. The
> status line previously said "Phases 0–5 complete" while the document control
> table said Phase 6 was complete; the two were reconciled there.
>
> **Revision 14 correction:** #24 is genuinely complete and closed. Commit
> `2f3f3a0` routes the legacy mean through the validated sample path, preserves
> its single warm-up, finalizes the existing `[Unreleased]` behavioral-correction
> paragraph and adds case 69. `67e37d7` reconciles the README; wiki commit
> `3a76948` documents #24 and repairs the stale #27 guidance missed by #27's own
> closure. The exact execution record is 80 cases, 643 assertions and zero
> failures; an independent rerun at `67e37d7` passes all 12 static checks.
>
> **Current scope:** #11, #29, #44, #45 and #46 are deferred to v1.4.1. v1.4.0 retains
> Office 32-bit compatibility branches but makes no 32-bit execution-
> certification claim.

> **Status:** v1.4.0 is published, tagged at `a5390b4`, certified against that exact SHA on 64-bit, with published assets re-hashed from the release; #29 is open in v1.4.1
>
> **Release theme:** Measurement integrity with explicit architecture-evidence boundaries
>
...
| Release tag | `v1.4.0` → `a5390b4c6ca56ebbd87eca121b5167ee5dc09963` |
| Published assets | `PERFORMANCE.MANAGER.xlsm` `5eddd647…` · `release-manifest.json` `d13567b4…`, both re-hashed from the release |
| Live issue snapshot | Seven closed v1.4.0 issues re-audited; #27 evidence chronology corrected `2026-08-31` |
| Open portfolio distribution | **19 total** · v1.4.0: **0** · v1.4.1: **12** · v1.5.0: **7** |
| Open-issue assignment | All 19 assigned to `danielep71` |
| Audit disposition | All seven v1.4.0 closures remain valid; #27 now records later wiki repair `3a76948`; #26 retains its disclosed process exception |
| v1.4.0 milestone | GitHub milestone `#3` |
| Open v1.4.0 issues | **0** |
| Closed v1.4.0 issues | **7**: #2, #23, #24, #25, #26, #27 and #28 |
| Explicitly deferred | #11, #29, #44, #45 and #46 → v1.4.1 milestone `#4` |
...
| Delivered implementation head | `67e37d737d09beb774a7cf8aaa3c48b00c7260fb` |
| Delivered so far | #26 (`6d44f2f`, `9362b8f`, `9064cff`) · #25 (`37c64fc`, manual dispatch PASS) · #23/#27 (`e5031fc`, `e7ab62d`) · #2 (`eff392f`, `e8eae90`) · #28 (`1c1f794`, `2c4f821`, `cdf2e4d`) · #24 (`2f3f3a0`, README `67e37d7`, wiki `3a76948`) |
| Current Excel regression | `67e37d7` · 80 cases · 643 assertions · 0 failures · Microsoft 365 Excel 64-bit Version 2607 Build 16.0.20228.20188 |
| Exact-tag 64-bit certification | `a5390b4` · 80 cases · 643 assertions · 0 failures · Microsoft 365 Excel 64-bit Version 2607 Build 16.0.20228.20188 |
| Exact-tag static certification | `a5390b4` · 12 checks · 0 failures · `branch: main` · `dirty: false` |
| Superseded pre-documentation run | `5a4d57b` · retained as history; not release evidence |
| Current static result | 12 checks · 0 failures · passed |
| Phase 6 status | **Complete.** Repository and wiki promoted to the canonical v1.4.0 contract; 1.4.0 stamps applied |
| Wiki promotion | `dd438f8` canonical promotion (11 pages) · `2d5a13d` suite counts scoped as a candidate snapshot · `eb1e77a` five stale harness-evidence claims corrected · `9dffa3f` no-valid-samples error description corrected |
| Phase 6 sign-off | Wiki `9dffa3f` · 12 static checks · 0 failures · source SHA is this commit |
| Version stamps | Advanced to `1.4.0` in the class, companion module, test harness and README |
| Wiki evidence (issue closure) | #23/#27: `052f903`, `f5ee791` · #28: `3f89a02` · #24 and missed #27 guidance: `3a76948` |
...
- prove the existing transactional QPC start correction on the aligned path;
- certify every architecture claimed by the release against the exact tagged SHA; v1.4.0 claims 64-bit execution only.

The headless Excel regression gate is not part of this milestone. Exact-SHA
Excel certification remains mandatory for the architecture actually claimed.
Office 32-bit certification is tracked separately in #29 for v1.4.1 and does
not block v1.4.0.
...
> complete aligned QPC start-failure regression coverage, and publish exact-SHA
> 64-bit Excel execution evidence. Office 32-bit certification is deferred to
> #29; headless Excel automation is tracked separately in #11 for v1.4.1.

## Scope decisions

| Decision | Resolution |
|---|---|
| [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) | **Closed.** `eff392f` added aligned strict/non-strict coverage and `e8eae90` asserted the exact strict error on both paths. Exact-SHA Excel evidence is 78 cases, 597 assertions and zero failures; all 12 static checks pass. No production or changelog change was required. |
...
| [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) | **Reopened and moved to v1.4.1.** It is an assurance follow-up, not a v1.4.0 release gate. |
...
| Fault injection | Workload start/endpoint seams landed with #23 and reach overhead sampling in `1c1f794`; `cdf2e4d` completes the total endpoint-failure regression and README reconciliation. |
| Baseline metadata | Delivered with #23 by forwarding the three optional outputs through the existing delegation. |
| Enum correction | Delivered under #26 without changing enum names or numeric values; the non-atomic changelog process exception remains disclosed. |
...
- The v1.4.1 automation/runtime/testability issues #11, #29–#36 and #44–#46.
- The v1.5.0 architecture and release-trust issues #37–#43.

## Release definition of done

- [x] All seven issues remaining in the v1.4.0 milestone satisfy every acceptance criterion and are closed with evidence; #29 is open in v1.4.1.
- [x] Forced QPC start failure is covered in strict and non-strict modes with `AlignToNextTick = True`.
- [x] No retained sample can belong to a backend other than the requested backend.
- [x] Workload, baseline and overhead vectors expose separate, coherent rejection and native-failure evidence.
- [x] Non-strict all-failed/all-rejected outputs are assigned before the aggregate error is raised.
- [x] Documentation and tests define the worker-raise evidence boundary by whether the harness classified the cycle; default outputs after an early raise are not treated as success.
- [x] `RejectedSamplesOut` is appended after the existing `FailedReadsOut` and `LastFailureStatusOut` optional parameters on `MeasureProcedure`, `MeasureBaseline` and `MeasureOverhead_Samples`, preserving positional and named existing calls.
- [x] Every behavior/API commit contains its own `[Unreleased]` entry; final documentation work does not reconstruct change history.
- [x] Legacy numeric overhead uses valid retained samples only.
- [x] `OverheadMeasurement_Text` renders only the expected no-valid-sample condition as undefined.
- [x] Workbook names with embedded apostrophes resolve through `Application.Run`.
- [x] Current source/docs state the real VBA `Long` enum semantics; v1.3.0 history is untouched.
- [x] Integrated regression passes in real 64-bit Excel against exact tag target `a5390b4`.
- [x] v1.4.0 makes no Office 32-bit execution-certification claim; #29 carries that unverified assurance surface into v1.4.1.
- [x] The exact-tag 64-bit run emits the frozen machine-readable result schema.
- [x] Static checks, workbook, tag, manifest and release evidence all bind to exact tag target `a5390b4`.

## Milestone issue register

| Order | Issue | Labels | Dependency |
|---:|---|---|---|
| 1 | [#26](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/26) — Correct enum type-safety claims to reflect VBA Long semantics | `bug` `documentation` `P2` `api` | **Closed** — `6d44f2f`, `9362b8f`, `9064cff` |
| 2 | [#25](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/25) — Escape apostrophes in workbook-qualified Application.Run targets | `bug` `P2` `measurement` | **Closed** — `37c64fc` + manual dispatch evidence |
| 3 | [#23](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/23) — Prevent backend fallback from mixing measurement sample vectors | `bug` `P2` `measurement` `api` `testing` | **Closed** — `e5031fc`, `e7ab62d` + wiki evidence |
| 3 | [#27](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/27) — Expose failure and rejection metadata from MeasureBaseline | `enhancement` `P3` `measurement` `api` | **Closed** — `e5031fc`, `e7ab62d` + wiki evidence |
| 4 | [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) — Failed QPC start read commits a zero-based session in non-strict mode | `bug` `P1` `timing` `testing` | **Closed** — `eff392f`, `e8eae90`; 78 cases, 597 assertions, 0 failures |
| 5 | [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) — Align MeasureOverhead_Samples fault injection and failure/rejection evidence | `bug` `P3` `measurement` `api` `testing` | **Closed** — `1c1f794`, `2c4f821`, `cdf2e4d`, wiki `3f89a02`; 79 cases, 629 assertions, 0 failures; 12 static checks pass |
| 6 | [#24](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/24) — Route OverheadMeasurement_Seconds through the validated sample path | `bug` `P2` `measurement` `api` | **Closed** — `2f3f3a0`, README `67e37d7`, wiki `3a76948`; 80 cases, 643 assertions, 0 failures |
| Deferred | [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) — Publish Office 32-bit execution certification | `enhancement` `release` `P2` `testing` | **Open in v1.4.1**; non-blocking for v1.4.0; not #11 |

## Dependency map

```mermaid
flowchart TD
  P0["Phase 0: contracts and schema"] --> I23["#23 + #27 complete"]
  P0 --> I2["#2 aligned-start regression"]
  P0 --> I26["#26 complete"]
  P0 --> I25["#25 complete"]
  I23 --> I28["#28 split seams and evidence"]
  I28 --> I24["#24 validated mean and text"]
  I2 --> INT["Integrated regression"]
  I26 --> INT["Integrated regression"]
  I25 --> INT
  I24 --> INT
  INT --> CERT["Exact-SHA 64-bit certification"]
  CERT --> REL["v1.4.0 tag and release"]
  REL -. "non-blocking assurance" .-> I29["#29 in v1.4.1"]
```
...
| Behavior/API change | Required same-commit `[Unreleased]` entry | Revision 18 status |
|---|---|---|
| #26 enum semantics | Finalize the `Changed` correction with the remaining source/wiki/example work | Delivered across `6d44f2f`, `9362b8f`, `9064cff`; historical atomicity exception disclosed |
| #25 target qualification | `Fixed`: apostrophe-containing workbook names now resolve | Complete in `37c64fc` |
| #23 + #27 backend/baseline | Behavioral correction for rejected fallback samples **and a separate `Added` entry** naming `MeasureProcedure.RejectedSamplesOut` plus baseline `FailedReadsOut`, `LastFailureStatusOut` and trailing `RejectedSamplesOut` | Complete in `e5031fc` |
| #28 overhead evidence | Separate `Added` entry naming `MeasureOverhead_Samples` `FailedReadsOut`, `LastFailureStatusOut` and trailing `RejectedSamplesOut`, plus a `Changed` behavioral correction for fallback rejection | Complete in `1c1f794` |
| #24 legacy overhead | Convert the existing planned behavioral-correction paragraph to delivered wording for validated means, the possible numeric raise and the text helper's explicit undefined result; no duplicate notice | Complete in `2f3f3a0` |
...
Every architecture-certification claim remains strict: every field in the
frozen schema is mandatory and `timestamp_utc` must contain the real observed
ISO-8601 UTC timestamp. v1.4.0 claims 64-bit execution only; #29 retains the
same contract for a future real 32-bit run.
...
## Phase 1 — [#26](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/26): Correct enum type-safety claims to reflect VBA Long semantics — complete

**Delivered:** `6d44f2f`, `9362b8f` and `9064cff`. The product and current
documentation criteria are satisfied. The issue's closing record explicitly
preserves the one historical process exception: the changelog finalization did
not land atomically with the first documentation correction and cannot be made
...
## Phase 2 — [#25](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/25): Escape apostrophes in workbook-qualified Application.Run targets — complete

**Delivered:** `37c64fc`, with the real apostrophe-named workbook dispatch
recorded as PASS. The policy deliberately normalizes leading/trailing spaces
through VBA `Trim$`; tabs, CR and LF are not normalized by that function and
the regression pins that boundary.
...
## Phase 3 — [#23](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/23) and [#27](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/27): requested-backend integrity and baseline evidence — complete

**Delivered:** implementation and contract documentation in `e5031fc`; missing
baseline degradation regressions and final repository reconciliation in
`e7ab62d`; release-first wiki corrections in `052f903` and `f5ee791`. The
verified implementation head passes 78 cases and 580 assertions in Microsoft
365 Excel 64-bit Version 2607 Build 16.0.20228.20188, with 12/12 static checks.
...
**Locked three-branch classification.** The fallback test and the invariant test
are separate branches, not one condition. Folding them together would record the
impossible `OK`-plus-mismatch state as an ordinary policy rejection and it would
never surface.

```text
StartStatus = cPM_ReadFallbackToMethod2   -> reject the cycle, skip the workload
StartedMethod <> ResolvedMethod           -> raise ERR_CPM_METHOD_MISMATCH
otherwise                                 -> run the workload, then classify the endpoint
```

**Enforced non-publication sentinel.** Locked after a regression showed the
documented boundary was unenforceable: `MeasureProcedure` zeroed its outputs on
entry, so a run that raised before classifying anything reported the same shape
as a flawless run.

```text
FailedReadsOut = -1        assigned before any validation that can raise
RejectedSamplesOut = -1
LastFailureStatusOut = cPM_ReadOK
```

- negative count: evidence was never published; inspect `Err`;
- non-negative counts: outputs are authoritative;
- `LastFailureStatusOut` is meaningful only while the counts are non-negative;
- the normal return and the aggregate raise overwrite the sentinels;
- `Err_Handler` must not overwrite already-published aggregate evidence;
- `MeasureBaseline` applies the same sentinel before its own blank-name check.

This is the same correction shape as CPM120-P2-02: a zero that means "no
observation" must not be storable where a zero that means "a real measurement"
is expected.

The mismatch raise is an **early internal-invariant raise**, not a worker raise:
the harness raises after inspecting the worker. Its evidence consequence is the
same. `Err.Number` and `Err.Description` are authoritative, the optional outputs
have not reached their publication point, and they must not be interpreted as
evidence. The description says "expected method" rather than "requested method",
because `ResolvedMethod` has already passed normalization.

A rejected cycle does not dispatch the workload. The resulting reduction in
workload-execution count is documented as part of the behavioral correction.
...
**Exit gate:** complete. Every returned workload and baseline vector is
requested-backend homogeneous; every rejected cycle is observable; the issue
bodies, README, changelog and wiki agree with the delivered branch state.

## Phase 3A — [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2): complete aligned QPC start-failure regression coverage — complete

The transactional production correction remains the v1.2.0 implementation in
`Start_TryCapture`. Commit `eff392f` extends the two existing strict/non-strict
QPC start-failure cases with `AlignToNextTick = True`, leaving
`TotalSteps = 78`. Commit `e8eae90` completes the evidence by asserting the
exact strict-mode error on both the aligned and unaligned paths.

### Delivered in `eff392f`

- Strict aligned failure preserves `HasActiveSession`, `ActiveMethodID`, `T1`
  and `RunLabel`, publishes `cPM_ReadQpcFailed` and proves one-shot recovery.
- Non-strict aligned failure commits a valid method-2 session, publishes
  `cPM_ReadFallbackToMethod2`, returns a sane elapsed value and proves one-shot
  recovery.
- The scope boundary names the later loop-read and timeout paths tracked in
  #44.
- The final recorded run at `e8eae90` is 78 cases, 597 assertions and zero
  failures on Microsoft 365 Excel 64-bit Version 2607 Build
  16.0.20228.20188, with all 12 static checks passing.

### Exact-error completion in `e8eae90`

Both strict subcases capture `Err.Number` before clearing it and assert exact
equality with `ERR_CPM_QPC_READ_FAILED` (`vbObjectError + 1007`). This closes
the false-positive gap in which any non-zero error previously satisfied the
regression. The aligned assertion is independent evidence because it passes
through `QPC_TryGetNextTick` rather than the unaligned direct-read path.

### Change boundary

- This is test-only unless the aligned failure exposes a runtime defect.
- Do not add a new `[Unreleased]` behavior entry for evidence-only coverage of
  a correction already shipped in v1.2.0.
- Later alignment-loop read failure and spin-guard timeout coverage belongs to
  [#44](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/44) in
  v1.4.1 and must not hold #2 or v1.4.0 open.
- If production code changes, stop, reopen the compatibility/changelog review
  and rerun every affected gate rather than treating the change as test-only.

**Exit gate:** complete. Both aligned modes pass in real 64-bit Excel at
`e8eae90`; the strict path asserts the exact error number; all 12 static checks
pass; and #2 is closed with links to the runtime fix, `eff392f`, `e8eae90` and
the final execution evidence.

## Phase 4 — [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28): Align MeasureOverhead_Samples fault injection and failure/rejection evidence — complete

### Delivered in `1c1f794`, `2c4f821` and `cdf2e4d`

1. The old ambiguous worker seam is retained as a documented endpoint-seam compatibility alias.
2. The locked start-read and endpoint-read seams apply consistently to `MeasureProcedure`, delegated `MeasureBaseline` and `MeasureOverhead_Samples`.
3. QPC and method-4 faults use their backend-appropriate low-level seams.
4. `MeasureOverhead_Samples` appends optional `FailedReadsOut`, `LastFailureStatusOut` and `RejectedSamplesOut` without moving existing parameters.
5. Native failures and fallback rejections are classified separately; wrong-backend observations never enter the vector.
6. Outputs are sentinelled before validation, published before the aggregate raise, and both injection counters clear on normal and raised exits.
7. Clean, partial endpoint-failure, total endpoint-failure, partial rejection, all-rejection, warm-up exclusion, strict worker-raise, recovery and legacy-call-shape regressions pass.
8. The strict worker-raise regression asserts exact `ERR_CPM_QPC_READ_FAILED` in `2c4f821`.
9. Distinct `[Unreleased]` `Added` and `Changed` entries landed with the implementation.
10. Wiki commit `3f89a02` records the release-first overhead evidence contract.
11. `cdf2e4d` adds the missing non-strict all-endpoint-failed subcase. It asserts exact `ERR_CPM_MEASURE_NO_VALID_SAMPLES`, `FailedReadsOut = 4`, `RejectedSamplesOut = 0`, `cPM_ReadQpcFailed`, publication before raise, cycle-specific diagnostics and recovery through the subsequent clean call.
12. The same commit reconciles the root README's release-first API/evidence guidance and removes both stale claims that overhead evidence was absent or #28 remained outstanding.
13. Exact-SHA evidence at `cdf2e4d` is 79 cases, 629 assertions and zero failures on Microsoft 365 Excel 64-bit Version 2607 Build 16.0.20228.20188; an independent rerun passes all 12 static checks.
...
The class header, README and wiki carry this equation and the same deliberate
warm-up boundary: native warm-up failures remain reliability evidence, while
fallback rejection applies only to measured cycles.

**Exit gate:** complete. The implementation, changelog, source guidance,
README, wiki and regression gates agree at `cdf2e4d`; #28 is closed and #24 may
begin.
...
**Exit gate:** complete. `2f3f3a0` implements the authoritative path and same-commit changelog conversion; `67e37d7` reconciles the README; wiki `3a76948` documents #24 and repairs the missed #27 guidance; 80 cases and 643 assertions pass with zero failures, and all 12 static checks pass independently.
...
| Transactional start | Direct and aligned QPC start failure in strict and non-strict modes; prior-session preservation; valid fallback commit; one-shot recovery |
...
### Evidence-only work receives no changelog entry

`#2` closed with no `[Unreleased]` entry, deliberately. It added regression
coverage for the aligned QPC start-failure paths of a correction that shipped in
v1.2.0; no production source changed and no behavior moved. A changelog records
what changed for a consumer, and nothing did.

A reader comparing closed issues against changelog entries will find `#2`
missing. That is the reason, recorded here so it does not read as an omission.
The rule generalizes: a commit that only adds or strengthens tests for
already-shipped behavior gets no entry. If a test exposes a defect requiring a
production change, the resulting change gets one.

### README limitation convention

Locked while closing #25, after the README was found asserting "Until #25 is
fixed" on a branch where it was.

- A limitation is described against the **shipped** version, because that is
  what an installed user has. It is not deleted when the branch fixes it.
- When the branch fixes it, the entry gains
  **"Fixed on the v1.4.0 development line (#nn)."**
- Phase 6 verifies that README **and wiki** limitation entries identify the
  released version they describe, identify fixes available on the v1.4.0
  development line, and do not refer to closed issues as unresolved.
- At release, promote the v1.4.0 signatures and behavior to the canonical
  documented surface on both.

The wiki is release-first: v1.3.0 remains its canonical API, with marked
development-line notes beside the released signatures rather than replacing
them.

There is no static check for this and there should not be: it is a judgment
about truthfulness, not a pattern. It is a checklist item instead.
...
- README limitation entries, against the locked convention below;
...
## Architecture-scope decision for #29

No real Office 32-bit host was available for v1.4.0. The explicit decision was
to retain the compatibility branches and support statement while limiting the
release-certification claim to the 64-bit host actually executed. #29 is open
in v1.4.1 and may be satisfied by a controlled contributor-provided 32-bit host.

This is an assurance deferral, not a behavioral or API change, so it receives
no changelog entry. It must never be rewritten as inferred certification or as
evidence that 32-bit is known to pass.
...
Repeat every affected exact-SHA static, claimed-bitness Excel, workbook and
...
### 7.4 Deferred Office 32-bit assurance

Office 32-bit execution is not a v1.4.0 gate. #29 remains open in v1.4.1 and
requires a controlled real 32-bit host, Win32 backend-2 execution, rollover
regressions, zero failures and the frozen SHA-bound JSON artifact before any
future 32-bit certification claim is made.
...
6. If a separate 32-bit artifact later exists, retain it separately because the current manifest records one bitness per invocation.
7. Attach workbook, manifest and the 64-bit execution-result artifact.
8. Publish only claims supported by the actual run.
...
5. Close every remaining milestone issue with direct evidence references.
6. Close the milestone.
7. Remove the merged release branch only after published evidence is verified.

## Implementation commit sequence

| Commit subject | Scope | Status/evidence |
|---|---|---|
| `docs(api): correct VBA enum semantics` | #26 class comment, example, README/wiki verification and changelog correction | **Complete** — `6d44f2f`, `9362b8f`, `9064cff`; historical atomicity exception recorded on #26 |
| `fix(measurement): harden workbook-qualified targets` | #25 code, real dispatch regressions and same-commit `Fixed` entry | **Complete** — `37c64fc` + manual dispatch PASS |
| `fix(measurement): enforce requested-backend integrity` | #23 + #27 start validation, worker-raise evidence, baseline outputs, behavioral correction and distinct `Added` entry | **Complete** — `e5031fc`, `e7ab62d`, wiki `052f903`/`f5ee791`; missed #27 guidance repaired in `3a76948` |
| `test(timing): cover aligned QPC start-failure paths` | #2 extends the existing strict/non-strict cases with `AlignToNextTick = True` and asserts the exact strict error; no production change | **Complete** — `eff392f`, `e8eae90`; 78 cases, 597 assertions, 0 failures; 12 static checks pass |
| `feat(measurement): expose overhead failure and rejection evidence` | #28 overhead three-output evidence, separate classification, cleanup, regressions and same-commit `Added`/`Changed` entries | **Complete** — `1c1f794`, `2c4f821`, `cdf2e4d`, wiki `3f89a02`; 79 cases, 629 assertions, 0 failures; 12 static checks pass |
| `fix(measurement): route legacy overhead through the validated path` | #24 authoritative path, preserved single warm-up, numeric raise, undefined text and same-commit planned-to-delivered behavioral-correction conversion | **Complete** — `2f3f3a0`, README `67e37d7`, wiki `3a76948`; 80 cases, 643 assertions, 0 failures; 12 static checks pass |
| `docs(release): reconcile v1.4.0 links and evidence wording` | README/wiki/releasing/install links, terminology and evidence references only; no first-time behavior entries | Pending after behavior work |
| `chore(release): prepare v1.4.0` | Final version stamps and release metadata after behavior is complete | Pending |

## Issue-closure evidence

| Issue | Minimum closure evidence | State at Revision 22 |
|---|---|---|
| [#2](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2) | Existing runtime correction plus forced aligned QPC start failure in strict/non-strict modes, exact strict error, transactional preservation/fallback, one-shot recovery and final exact-SHA Excel/static evidence | **Closed** — `eff392f`, `e8eae90`; 78 cases, 597 assertions, 0 failures; 12 static checks pass |
| [#23](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/23) | QPC/method-4 fallback, strict forced-start early worker raise with `Err.Number` evidence, homogeneous vectors, separate failure/rejection counters, accurate message and same-commit changelog entry | **Closed** |
| [#24](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/24) | Shared-path review; endpoint/fallback/no-valid regressions; graceful text result; explicit `[Unreleased]` planned-to-delivered conversion; README and wiki reconciliation | **Closed** — `2f3f3a0`, `67e37d7`, wiki `3a76948`; 80 cases, 643 assertions, 0 failures; 12 static checks pass |
| [#25](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/25) | Real apostrophe-named workbook dispatch, qualification matrix and same-commit `Fixed` entry | **Closed** |
| [#26](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/26) | Correct source/docs, practical boundary example and green released-history check | **Closed**; historical atomicity exception disclosed |
| [#27](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/27) | Forwarded three-output evidence, worker-raise boundary, clean/partial/fallback/all-failed baseline regressions and distinct same-commit `Added` entry | **Closed** — `e5031fc`, `e7ab62d`, wiki `052f903`/`f5ee791`; missed guidance corrected in `3a76948` |
| [#28](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28) | Separate seams, three overhead outputs, separate count semantics, clean/partial/total/rejection regressions, worker-raise boundary, cleanup proof, README/wiki/header agreement and distinct same-commit `Added`/`Changed` entries | **Closed** — `1c1f794`, `2c4f821`, `cdf2e4d`, wiki `3f89a02`; 79 cases, 629 assertions, 0 failures; 12 static checks pass |
| [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) | Completed exact-SHA real 32-bit JSON artifact with zero failures and explicit Win32 path coverage | **Open in v1.4.1**; non-blocking for v1.4.0 |
...
- Any claim that v1.4.0 was executed or certified on Office 32-bit.
...
### [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) — Publish Office 32-bit execution certification

`#29` is reopened and assigned to v1.4.1. The source retains 32-bit
compatibility branches, but v1.4.0 carries no real-host 32-bit execution
evidence. A controlled contributor-provided host is acceptable; inferred
...
### [#44](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/44) — Add deterministic alignment-loop failure and timeout coverage

`#44` is assigned to v1.4.1. It adds deterministic test-only coverage for a
failed alignment-loop read after initial success and for spin-guard exhaustion
in the QPC and method-4 paths. #2 remains limited to the reachable initial QPC
alignment-read failure and does not wait for #44.

### [#45](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/45) — Add compile-safety checks for error labels and undeclared assignments

`#45` is assigned to v1.4.1. It adds two separately reported, fixture-backed
static checks: every non-special `On Error GoTo` target must resolve within its
procedure, and every supported unqualified assignment target must resolve to an
allowed declaration. The intended gate advances from 12 to 14 checks only when
the implementation and all current count surfaces land together.

### [#46](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/46) — Harden label synchronization and upgrade github-script

`#46` is assigned to v1.4.1. It requires a complete paginated label preflight,
one sorted missing-label report with no partial mutation or automatic creation,
and the `actions/github-script@v8` Node 24 runtime upgrade. Revision 21 records
the issue only; the workflow is unchanged.

## Appendix A — Revision 21 live open-issue reconciliation

The 31 August 2026 reconciliation incorporates #29's evidence-based reopening,
#36's corrected scope and new assurance issues #45/#46. Nineteen issues remain
open and scheduled: none in v1.4.0, twelve in v1.4.1 and seven in v1.5.0. All 19
are assigned to `danielep71`.

### Corrections applied directly on GitHub

- #2 is closed after `e8eae90` added the exact strict-error assertions. Its
  body and closing comment record the `eff392f`/`e8eae90` evidence, 78 cases,
  597 assertions, zero failures and 12 passing static checks.
- #24 is closed after `2f3f3a0` delivered the validated legacy mean/text path,
  preserved the single warm-up and converted the existing `[Unreleased]`
  paragraph from planned to delivered. README `67e37d7` and wiki `3a76948`
  complete the documentation; the wiki commit also repairs the missed #27 gap.
  Final evidence is 80 cases, 643 assertions, zero failures and 12 passing static
  checks.
- #28 is closed after `cdf2e4d` supplied the missing all-endpoint-failed
  regression and reconciled the root README. Its final record binds the
  implementation (`1c1f794`, `2c4f821`), wiki (`3f89a02`) and exact-SHA Excel
  result of 79 cases, 629 assertions and zero failures; all 12 static checks
  pass independently.
- #29 is reopened in v1.4.1. It records that the standalone schema and
  controlled manual route are frozen while making clear that no real 32-bit
  execution artifact exists yet.
- #31 now carries `state-management`.
- #36 now states that #27 and #28 delivered the three evidence outputs on both
  delegated measurement APIs; its stale bug-report-template statement remains
  the fifth live contradiction, while #45/#46 stay outside its scope.
- #37, #38 and #39 now require v1.5.0 source compatibility and defer any
  incompatible removal or signature break to v2.0.0.
- #40 now carries `api` and applies the same minor-release SemVer boundary to
  the worksheet-reporting extraction.
- #42 is raised from `P3` to `P2` and is explicitly sequenced before #37–#40 so
  the public API, errors, tests, static checks and source inventory form a
  baseline before extraction.
- #43 is retitled to focus on signing, attestations and enforceable artifact
  integrity because the immutable-artifact policy already exists.
- #44 now owns deterministic alignment-loop read-failure and timeout coverage
  for QPC and method 4. It is explicitly deferred to v1.4.1 and cannot block
  #2 or v1.4.0.
- #45 now owns the two compile-safety lint rules as separately reported checks.
- #46 now owns aggregate missing-label diagnostics and the Node 24 action
  runtime upgrade; no workflow change landed during issue creation.
- All open issues are assigned to `danielep71`; closed #2 and #24 remain
  assigned for historical ownership.

### Full open-issue register after audit

| Issue | Milestone | Labels | Audit disposition |
|---|---|---|---|
| [#11](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/11) — Add a headless Excel regression gate | v1.4.1 | `enhancement` `CI` `P2` `testing` | Valid; automation remains independent of v1.4.0 manual certification |
| [#29](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/29) — Publish Office 32-bit execution certification | v1.4.1 | `enhancement` `release` `P2` `testing` | Reopened; non-blocking for v1.4.0; real-host certification artifact pending |
| [#30](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/30) — Make ElapsedTime robust for large finite durations | v1.4.1 | `bug` `P3` `timing` `api` | Valid; large-duration `Long` conversion remains unsafe |
| [#31](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/31) — Roll back timeBeginPeriod when aligned method-3 start fails | v1.4.1 | `bug` `P3` `timing` `state-management` | Valid; failed aligned start can retain timer-period ownership |
| [#32](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/32) — Harden statistics arithmetic for extreme in-domain values | v1.4.1 | `bug` `statistics` `P3` `api` | Valid; mean, median and squared-deviation arithmetic remain vulnerable |
| [#33](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/33) — Add deterministic TW failure and workbook-lifecycle regression coverage | v1.4.1 | `enhancement` `P2` `state-management` `testing` | Valid; deterministic seams and lifecycle coverage remain absent |
| [#34](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/34) — Define the minimum supported Office/VBA version | v1.4.1 | `documentation` `P3` `api` | Valid; legacy conditional branches remain without a formal support floor |
| [#35](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/35) — Harden TW instance-key issuance against Long overflow | v1.4.1 | `bug` `P3` `state-management` | Valid; the `Long` key seed still lacks an overflow policy |
| [#36](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/36) — Reconcile remaining source, repository, and CI contract contradictions | v1.4.1 | `bug` `documentation` `CI` `P3` | Corrected; five confirmed contradictions remain; #45/#46 explicitly separate |
| [#37](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/37) — Extract native timing internals behind the stable class facade | v1.5.0 | `refactor` `P3` `timing` | Valid; extraction must preserve the v1.5.0 public contract |
| [#38](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/38) — Extract the measurement harness into a dedicated module | v1.5.0 | `refactor` `P3` `measurement` | Valid; extraction must preserve measurement call shapes and behavior |
| [#39](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/39) — Extract the statistics engine into a dedicated module | v1.5.0 | `statistics` `refactor` `P3` | Valid; extraction must preserve statistics signatures and errors |
| [#40](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/40) — Move worksheet report rendering into a dedicated reporting module | v1.5.0 | `refactor` `P3` `api` | Valid; v1.5.0 cannot silently remove supported call shapes |
| [#41](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/41) — Split the regression suite into headless domain modules and an optional UI wrapper | v1.5.0 | `CI` `refactor` `P2` `testing` | Valid; suite modularization remains open |
| [#42](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/42) — Generate API, error, test, static-check, and source inventories | v1.5.0 | `enhancement` `CI` `P2` `api` | Valid; prerequisite baseline for #37–#40 |
| [#43](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/43) — Strengthen release trust with signing, attestations, and enforceable artifact integrity | v1.5.0 | `enhancement` `release` `P2` | Valid; remaining scope is enforceable integrity and provenance |
| [#44](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/44) — Add deterministic alignment-loop failure and timeout coverage | v1.4.1 | `enhancement` `P3` `timing` `testing` | Valid; owns post-initial-read failure and spin-guard coverage without blocking v1.4.0 |
| [#45](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/45) — Add compile-safety checks for error labels and undeclared assignments | v1.4.1 | `enhancement` `CI` `P2` `testing` | New; two distinct fixture-backed checks; planned static count 12 → 14 |
| [#46](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/46) — Harden label synchronization and upgrade github-script | v1.4.1 | `bug` `CI` `P3` | New; aggregate fail-before-mutation label audit plus `github-script@v8` |

### Revision 7 closure delta retained — historical

Revision 6 reproduced #24–#29. Revision 7 added the complete current record
for #2 because the closure audit moved it into the v1.4.0 milestone. The body
snapshot below predates `eff392f` and the Revision 13 verification; current
state is recorded in Phase 3A and the live issue.

### #2 — Failed QPC start read commits a zero-based session in non-strict mode

- **URL:** https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/2
- **State:** closed · completed
- **Milestone:** v1.4.0 (GitHub milestone #3)
- **Labels:** `bug` `P1` `timing` `testing`
- **Assignee:** `danielep71`
- **Last updated:** `2026-08-30T15:31:19Z`

<details>
<summary><strong>Historical GitHub issue body at Revision 7</strong></summary>

## Status — reopened after closure audit

The runtime defect was fixed in v1.2.0, but one explicit regression criterion
was checked prematurely: forced QPC start failure has not been exercised with
`AlignToNextTick = True`.

## Defect

Before v1.2.0, a failed QPC start read in non-strict mode could commit method 5
with `m_qpcStart = 0`. A later successful endpoint read then produced a
machine-uptime-like duration instead of the operation's runtime. The
aligned-start branch had the same risk.

## Delivered runtime correction

Source commit
[`196bb19dd50128cd83ec612fa5c123a4c0a3136a`](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/commit/196bb19dd50128cd83ec612fa5c123a4c0a3136a)
made start capture transactional through `Start_TryCapture`:

- strict failure raises without replacing the previously committed session;
- non-strict failure captures and commits a valid method-2 start before
  changing session state;
- `ActiveMethodID` exposes method 2 and `LastReadStatus` exposes
  `cPM_ReadFallbackToMethod2`;
- failed reads never enter timestamp arithmetic.

Regression commit
[`a4df858520dcd7ad55aa762bb814b19e1bca07b9`](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/commit/a4df858520dcd7ad55aa762bb814b19e1bca07b9)
added strict and non-strict unaligned QPC start-failure cases. The released
v1.2.0 suite recorded 63 cases, 431 assertions and 0 failures on Microsoft 365
Excel 64-bit Version 2606 Build 16.0.20131.20152.

## Remaining regression gap

The fault-injection cases call `StartTimer` with the default/false alignment
argument. Ordinary aligned QPC starts are exercised elsewhere, but no test
combines:

```text
forced QPC start failure
+ AlignToNextTick = True
+ strict/non-strict policy
```

Because this gap is directly repairable, it is not treated as a historical
process exception. Add a deterministic aligned-start failure regression,
verify transactional preservation/fallback, rerun the exact-SHA gates, and
then close.

## Acceptance criteria

- [x] No code path commits a session after a failed start read.
- [x] Non-strict QPC start failure binds method 2 and `ActiveMethodID` reports 2.
- [x] Strict QPC start failure preserves the prior session state exactly.
- [ ] Injected-failure regression tests cover both modes and the aligned path.
- [x] A test asserts elapsed time is never computed from a sentinel start.

</details>

## Appendix B — Revision 6 issue snapshot and open-body verification

> **Historical reference only — do not use this appendix for current state or
> unchecked criteria.** The text below was reproduced from GitHub on 27 August
> 2026 before #23/#27 closed and before #2 re-entered the milestone. The
> Revision 13's milestone register, Appendix A and live GitHub records are
> authoritative. The embedded #24/#28/#29 bodies below predate the Revision 8
> refinements and are retained only as historical snapshots; do not use their
> unchecked criteria as current state. The #23/#25/#26/#27 entries likewise
...
### #27 — Expose failure and rejection metadata from MeasureBaseline
...
### #28 — Align MeasureOverhead_Samples fault injection and failure/rejection evidence

- **URL:** https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/28
- **State:** open
- **Milestone:** v1.4.0 (GitHub milestone #3)
- **Labels:** `bug` `P3` `measurement` `api` `testing`
- **Last updated:** `2026-08-30T08:30:29Z`
...
## Appendix C — Refresh checkpoints

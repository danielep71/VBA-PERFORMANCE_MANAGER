...
| Pull-request publication | PR #61 carries implementation; it need not merge for plan synchronization. A separate documentation-only PR supplies the `main` pointer. All protected-main requirements remain unchanged |
...
The working branch is proposed through existing PR #61. Revision 8 required
...
adds the plan-location policy and the #51 fixture-harness invocation arrangement
below, before Revision 6 reaches `main`. The second commit is the canonical
Revision 6; the first is superseded by it and never reached `main`.

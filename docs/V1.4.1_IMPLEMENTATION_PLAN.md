...
| Current static gate | 12 checks; #51 adds `release provenance strict fixtures` (→13), #50 adds release-state controls, #45 adds two compile-safety checks |
...
The `protect-main` ruleset requires all changes to `main` to arrive through a
pull request, with no documentation exemption. Plan updates are committed on
the working branch and merged to `main` by pull request like any other change.

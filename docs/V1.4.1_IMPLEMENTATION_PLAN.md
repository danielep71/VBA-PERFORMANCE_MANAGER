...
| Current static gate | 13 checks on `v1.4.1`, including #51's `release provenance strict fixtures`; `main` remains at 12 until merge; #50 and #45 add later controls |
...
Advance the active gate by two from the post-#50 count. The chain is 12 → 13
after #51 → 13 plus #50's implemented count → plus two here. #50's contribution
is recorded only when implemented and is never guessed in advance.

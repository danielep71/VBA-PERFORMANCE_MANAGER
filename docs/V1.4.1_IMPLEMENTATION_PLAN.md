...
> [!IMPORTANT]
> During the active v1.4.1 milestone, the
> **[plan on `v1.4.1`](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/blob/v1.4.1/docs/V1.4.1_IMPLEMENTATION_PLAN.md)**
> is canonical. Any copy on `main` is a historical snapshot, not the live
> planning authority. This notice supersedes older location/publication rules
> in the revision history below. At release completion, merge the final plan
> to `main` and archive it under `docs/releases/v1.4.1/`.

> **REVISION 13 — 8 September 2026**
>
> **Status:** Active planning baseline.
>
> **Source baseline:** `abee10b609339fddc7f8ef0ac3fcfca4f8dbfd42`
> on `main`; review-cleanup baseline on `v1.4.1` is
> `81a8ecdcb0c4ddc05055e48be4530266fa8c58c7`, including completed #51.
>
...
> body, label, assignment or milestone change must update and push this plan on
> `v1.4.1` in the same task. This does not require merging implementation to `main`.
...
| Active issues | **12**: #11, #29–#35, #44, #46, #47, #50 |
| Full issue inventory | **47 total** · **20 open** · **27 closed** |
| Open portfolio distribution | v1.4.1: **12** · v1.5.0: **8** |
| Closed portfolio distribution | v1.2.0: **4** · v1.3.0: **11** · v1.4.0: **7** · v1.4.1: **4** (#36, #45, #49, #51) · no milestone: **1** (#22, `not planned`) |
...
| Plan location | Canonical on `v1.4.1` during the active milestone; any `main` copy is historical; final plan merged to `main` and archived under `docs/releases/v1.4.1/` at release completion |
| Working branch | `v1.4.1`, based on `abee10b6` |
| Pull-request publication | Pointer PR #62 merged to `main` at `bb5c5528`; obsolete PR #61 closed unmerged. Implementation remains on `v1.4.1`; open a new release PR when ready |
| Current static gate | 15 checks on `v1.4.1`: #51's provenance fixtures plus #45's two compile-safety checks; `main` remains at 12 until merge; #50 adds later controls |
...
## Revision 13 — #36 closed; published wiki baseline verified

The owner published the 15 heading-link corrections across ten files at wiki
`1039253ca541f5b8948563496ae666691dfbc9f9`. Every replacement file is byte-identical
to the supplied anchor-fix bundle. The preceding content reconciliation is
`f18fd82fa440c41a7f1b0584cba9954519dc5188`; repository corrections are in
`d2794d6ae34f72592f6ec49ce66df5d02f014a53`, followed by temporary-patch removal
in `ee7aea95ecdfb5ea810c1f07396aefa189cea97c`.

All 215 wiki links checked across 20 rendered content pages resolve, including
the formerly missing U+FE0F fragments. All 72 relative links checked in the
four repository documents and this plan also resolve. The complete published wiki tree preserves the reviewed
release content and historical evidence; the final change is exactly 15 link
replacements. The repository's relative links and historical patch link are
valid. All 15 source checks and diff-whitespace checks pass; no runtime or
workflow changes and no new Excel execution are claimed.

#36 is closed as completed with every acceptance criterion checked and an
exact-SHA verification comment. #45 remains closed. The content prerequisite
for #50 is now met; its initial pinned wiki baseline is the full `1039253` SHA
above. #34 remains a separate support-policy decision, not a reason to leave
completed factual reconciliation open. Later policy changes require their own
corresponding documentation refresh.

The portfolio now has 47 issues: 20 open and 27 closed. v1.4.1 has 12 open and
four closed (#36, #45, #49, #51). The working-branch plan remains canonical;
main integration is deferred to the eventual release PR. Earlier open/blocker
statements in Revisions 11 and 12 are historical and superseded here.

## Revision 12 — wiki published; #45 closed; #36 rendered-link gate

The owner published the complete wiki replacement files at
`f18fd82fa440c41a7f1b0584cba9954519dc5188`. Its Git tree is byte-identical
to prepared `b417bca`: all ten content corrections and two badge-anchor repairs
are now live. Source cleanup `ee7aea95ecdfb5ea810c1f07396aefa189cea97c`
deletes only the temporary patch; the historical link below now pins its
retained version at `d2794d6` rather than pointing to a deleted working file.

#45 is closed as completed: the published wiki now explicitly distinguishes
15 active v1.4.1 checks from 12 on main and the immutable release evidence.
At clean source `ee7aea9`, the JSON gate reports 15 checks, zero failures,
`dirty: false`; 41 compile-safety fixtures and three isolated real-CLI/JSON
scenarios pass. The implementation is unchanged. PR #61 was closed unmerged;
implementation reaches main through the eventual release PR, not as a condition
for issue closure. No new Excel execution is claimed.

#36 remains open for its existing relative-link criterion. Reading GitHub's
rendered HTML found 15 heading-link occurrences across ten wiki pages that
omit U+FE0F, an invisible variation selector retained in the actual heading
anchors. The checked-in Markdown headings look correct, but the links miss
them. The affected pages are Architecture-and-Internal-Design,
Diagnostics-and-Benchmark-Helpers, Error-Handling-and-Strict-Mode,
Examples-and-Usage-Patterns, Execution-Control-and-Environment, Home,
Installation-and-Import, Known-Limitations, Strict-Mode-and-Error-Behavior,
and Testing-and-Validation. Complete replacement files with only the 15 URL
fragments corrected were supplied for manual wiki publication. There is no
new runtime scope and the source repository does not recreate the removed patch.

All source/template, repository-document and wiki content criteria are now
satisfied. Full rendered-link verification and the final closure-SHA gate
remain open. #50 still follows #36. Earlier publication-blocker statements in
Revision 11 are historical and superseded by this record. Portfolio totals
are now 47 issues, 21 open and 26 closed; v1.4.1 has 13 open and three closed.

## Revision 11 — #36 reconciliation; wiki publication remains open

The audit baseline is `9fa0d738ebba3c601afb2bd512f64d8fbceeda56` on
`v1.4.1`. README now names the working branch as development authority;
INSTALLATION identifies the released two-file package, official assets and
64-bit-only certification; RELEASING restores the explicit bitness rule and
records the dated v1.4.0 deferral. SECURITY's neutral matrix was re-verified
without modification. T2/ET, LastReadStatus and overhead source headers are
corrected; all non-comment class lines and CRLF encoding are unchanged.
The unavailable Discussions contact now links to the usage FAQ. Bug-report
output/sentinel guidance and the earlier review-follow-up fixes remain valid.

The live wiki is still `9dffa3f78136dbc9ab6f0debf25a32bf8b812a2b`.
The complete correction is retained in
[historical issue-36-reconciliation.patch](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/blob/d2794d6ae34f72592f6ec49ce66df5d02f014a53/docs/wiki-pending/issue-36-reconciliation.patch),
from that base to local wiki commit `b417bca` (including predecessor `5dea355`).
It reconciles ten content pages, including the repeated #33 limitation in
Time-Waster Suppression, and fixes two additional badge anchors in Benchmarking
Guidance and Timer Methods. FAQ's alleged duplicate hard-End sentence was
already absent; its release badge and upgrade guidance still required edits.

Wiki push cannot authenticate in this environment. The patch is prepared and
validated, **not published**. From an authenticated, clean wiki checkout at the
recorded base, apply the retained patch with `git apply --check <patch-path>`,
then `git apply <patch-path>`, review, commit and push. If the wiki has moved,
reconcile against its new head first. Re-fetch the published SHA and validate
it before checking any wiki delivery criterion or closing #36. #45's remaining
wiki-count criterion also stays open; #50 cannot use the old published wiki as
a corrected baseline.

Validation: all 15 static checks pass; source and wiki diffs pass
`git diff --check`; relative file/page targets across the four repository
documents and all 22 prepared wiki pages exist, and two stale badge anchors
were corrected. Full rendered-anchor verification remains part of the
published-wiki gate. The patch applies to
its recorded base and reproduces the prepared wiki tree. No Excel execution,
workflow change, tagged-evidence rewrite or issue closure is claimed. Issue
counts and assignments are unchanged. Final checks at #36's eventual closure
SHA and published wiki verification remain required.

Revision 11 supersedes earlier live-status statements below; those revision
sections are history, including their then-current check counts and policies.

## Revision 10 — pointer publication and obsolete PR retirement

PR #62 was already open; no duplicate was created. Its eight-line authority
notice in one Markdown file passed the hosted checks and merged through the
normal protected PR route at `bb5c5528c14c8f9e8fd834f5502c67b781c8ecac`.
`main` now honestly identifies its Revision 5 copy as historical and links to
the canonical plan on `v1.4.1`. No #51/#45 implementation was merged.

PR #61 was closed unmerged, not kept as an ambiguously scoped release PR. Its
body now records that disposition and corrects the false claim that `main`
held Revision 6. All implementation remains on `v1.4.1`; existing issue states
and exact-commit evidence are unchanged. A new release PR can be opened when
the release scope and candidate are ready. Branches were not deleted.

Revision 6's publication narrative below is explicitly historical; the old
future-tense main-publication expectation is removed. #36 is the next planned
reconciliation task. Its wiki publication blocker and #45's remaining wiki-count
criterion are not resolved by this planning cleanup.

## Revision 9 — active-milestone planning authority

The owner approved replacing Revision 6's main-canonical policy. During an
active milestone, its plan is canonical on the designated working branch.
Every issue change must update and push that plan in the same task. A copy on
`main`, if present, is a historical snapshot, not the live planning authority.
At release completion, the final plan is merged to `main` and archived under
`docs/releases/<version>/`.

For this milestone, the designated branch is `v1.4.1` and the active path is
`docs/V1.4.1_IMPLEMENTATION_PLAN.md`. The working implementation baseline is
`fbaf3a477c45e4adf4a080655d675e5b44f533e5`; `main` at `abee10b6` still holds
Revision 5. Its three-revision lag under the former policy is recorded openly,
not described as prior compliance. A documentation-only pointer is proposed
from `main` without bringing across #51/#45 implementation.

Revision 6's location policy and the main-publication obligations stated in
Revisions 7–8 are superseded. Historical revision narratives remain below for
traceability. PR #61 need not merge merely to synchronize the plan. Issue
acceptance criteria, closure evidence, certification and protected-branch
controls are unchanged. #45's wiki-count criterion remains open; this policy
does not make an unpublished wiki patch satisfy it.

## Revision 8 — #45 compile-safety implementation

The user explicitly selected #45 next, ahead of #36/#50. This overrides the
earlier sequencing, not either issue's scope: compile-safety fixtures do not
depend on the wiki or release-state checker. The active count is **13 → 15**,
not the stale 12 → 14 in #45's original body. #50's eventual contribution remains
unspecified until implemented.

- `tools/vba_compile_safety.py` checks same-procedure handler labels and declared
  assignment targets across every VBA export in `src/`, `test/` and `demo/`.
- Pre-VBA7, VBA7 Win32 and VBA7 Win64 profiles are analysed separately; inactive
  labels/declarations cannot satisfy active references. Unknown conditional
  syntax fails rather than silently bypassing checks.
- 41 positive/negative fixtures run inside the two ordinary gate results;
  three isolated real-CLI scenarios verify exit codes and independent JSON
  failure results. The full static result is **15/15**.
- [Scanner contract](COMPILE_SAFETY_CHECKS.md) defines supported grammar,
  explicit ReDim declarations, source-line diagnostics and compiler limitations.
- README current-branch counts and the Unreleased tooling entry are updated.
  Published v1.4.0 12/12 evidence and archived plans remain unchanged.
- No VBA, workbook or workflow file is changed; no new Excel execution is
  claimed. The issue comment carries the independently reverified published
  SHA and test results. #45 remains open for the current-count documentation
  criterion: repository counts are updated, but the live wiki is not.

At Revision 8, the working branch was proposed through PR #61 (since closed
unmerged in Revision 10). Revision 8 required
canonical-main synchronization through its protected merge; Revision 9
supersedes that planning requirement, not the merge controls. The wiki correction
prepared earlier remains unpublished because wiki Git authentication is blocked;
#36, #45 and #50 remain open. The earlier wiki patch must also be adjusted for
the now-15-check branch before publication. No wiki completion is inferred from
#45 and no acceptance criterion is weakened to enable closure.

## Revision 7 notification and review reconciliation

The audit inspected review threads across all thirteen existing pull requests;
it did not have access to the maintainer's personal notification inbox.

- #51 is closed on implementation/evidence through `e92cfb46780e7a8c8e55efeb0697f09924bbbbdf`:
  33 provenance fixtures and 13 static checks. This code is on `v1.4.1`, not yet
  on `main`. The historical Revision 6 implementation instructions below remain
  design history, not an assertion that #51 is still pending.
- PR #56 was abandoned without merging. Three unresolved pin-gate findings are
  explicitly transferred to open #46: parsed YAML coverage (quoted keys and flow
  mappings), recursive local composite-action coverage, and complete 64-hex
  Docker digest validation. They are not implemented by this documentation pass.
  The old line-ending finding is obsolete on that abandoned diff; a fresh
  checkout of `81a8ecd` is clean. Revalidate normalization when reviving the code.
- PR #59's artifact-lineage finding is corrected in `RELEASING.md`: a changed
  post-merge SHA requires new static/Excel validation, workbook rebuild,
  packaged test and hash before tagging. Pre-merge evidence remains historical.
  This is a procedure correction, not new Excel or workbook certification.
- #36 records the PR #60 template corrections (ordinary-Markdown closing
  references and absolute documentation links) and PR #57's missing changelog
  maintenance-policy entry, implemented in this revision's change set.
  Previously checked repository-document criteria require the full
  re-verification already mandated by Revision 6; #36 remains open.
- #50 explicitly retains PR #57's historical comparison-link-chain finding.
  Correction and fixtures must respect the existing released-section freeze;
  this pass does not rewrite historical release bodies or add linter rules.

Issue states, labels, assignees and milestones are unchanged by this audit.
Only #36, #46 and #50 receive review-follow-up/body corrections. No runtime,
workflow, published tag, binary or archived plan was changed. Under the policy
then in force, Revision 7 required a protected-main merge for canonical plan
publication. Revision 9 supersedes that planning requirement.
...
5. **Harness placement and gate wiring.** The matrix lives in
   `tools/test_release_provenance.py` and is invoked by `vba_lint.py` as one
   separately reported check named `release provenance strict fixtures`, per the
   invocation arrangement recorded above. The module is also runnable standalone
   to produce the closure matrix. `static-checks.yml` is not modified.

### Former plan location policy — superseded by Revision 9

The following records Revision 6's policy, not the active rule. Revision 9
makes the designated working branch canonical during the active milestone.

The active implementation plan is the repository-wide planning authority, not a
branch-local working note. Issues and milestones are repository-wide, so the
same-task synchronization rule is not satisfied when only the working branch
holds the truthful plan.

| State | Location |
|---|---|
| Active implementation plan | Canonical on `main` |
| Working branch | Carries the same plan plus implementation in progress |
| Completed release plan | Frozen under `docs/releases/` |

The ruleset protecting `main` requires all changes to arrive through a pull
request, with no documentation exemption. Plan updates are committed on the
working branch and merged to `main` by pull request like any other change.

### Publication of Revision 6 — historical record only

Revision 6 was published in two commits. `243f2985c46bd19d1046d2c6b2245eeb893cb4db`
landed the reconciliation and the #33 re-sequencing on `v1.4.1`. A second commit
added the then-applicable plan-location policy and the #51 fixture-harness
invocation arrangement below. It superseded the first Revision 6 snapshot on
that branch. Revision 6 did not reach `main`; its anticipated publication there
was an expectation under the former policy, not a current instruction.
Revision 9 superseded that policy and Revision 10 records the merged main pointer.

### #51 fixture-harness invocation

The repository has no Python test tree, and `vba_lint.py` carries its fixtures
inline. #51's matrix cannot be inline: it asserts process exit codes against a
copied script in throwaway repositories. The arrangement is therefore delegated
rather than inlined:

```text
static-checks.yml
    → vba_lint.py
        → tools/test_release_provenance.py
            → copied release_provenance.py in temporary repositories
```

Consequences:

- production release code gains no `--selftest` path, preserving the single
  strict entry point and the two-tier exit-code contract;
- the full fixture matrix stays cleanly separated in its own module, and the
  standalone command also produces the detailed closure matrix;
- `static-checks.yml` is unchanged, so #51's workflow boundary holds;
- the matrix is gate-enforced from the moment #51 closes, rather than resting
  on procedural care;
- a non-zero harness result is an ordinary static-gate failure;
- the check is reported separately as `release provenance strict fixtures`, and
  raises the active gate from 12 to 13.

`vba_lint.py` already validates changelog and workflow policy, so one delegated
repository-tooling check is consistent with its current role despite its
historical name.
...
| Done | [#51 — Reject incomplete or inconsistent release provenance manifests](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/51) | `bug` `CI` `P1` `release` `testing` | Closed with 33 fixtures and 13 static checks through `e92cfb4` on `v1.4.1`; merge to `main` pending |
...
| Done | [#36 — Reconcile v1.4.0 post-release source, repository, and wiki contradictions](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/36) | `bug` `documentation` `P1` `release` | Closed after repository reconciliation in `d2794d6`, wiki content publication at `f18fd82`, and all 15 heading-link corrections at `1039253`; rendered wiki links verified |
| 3 | [#50 — Add release-state consistency checks for changelog, README and wiki](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/50) | `enhancement` `documentation` `CI` `P2` `release` `testing` | Documentation prerequisite #36 is complete; use verified wiki `1039253ca541f5b8948563496ae666691dfbc9f9` as the initial pinned baseline |
| Closed | [#45 — Add compile-safety checks for error labels and undeclared assignments](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/45) | `enhancement` `CI` `P2` `testing` | Closed after published wiki count verification at `f18fd82`; 41 fixtures, three CLI scenarios and all 15 checks pass at `ee7aea9`. Implementation remains on `v1.4.1`; #61 closed unmerged |
| 4 | [#46 — Pin workflow actions immutably and harden label synchronization](https://github.com/danielep71/VBA-PERFORMANCE_MANAGER/issues/46) | `bug` `CI` `P2` | Independent workflow correction; edits the hosted static workflow, so sequence its merge deliberately |
...
The five locked implementation decisions above govern exit codes, the absence of
a draft mode, `HEAD` identity, the fixture harness and its gate wiring. Fixture
coverage is the complete sixteen-item negative matrix in the issue plus a
positive v1.4.0-equivalent invocation, with both text and JSON success output
verified.

The matrix lives in `tools/test_release_provenance.py` and is delegated from
`vba_lint.py` as `release provenance strict fixtures`, raising the active gate
from **12 to 13**. A non-zero harness result is an ordinary static-gate failure.
`static-checks.yml` is not modified, so the issue's workflow boundary holds.
...
- README, INSTALLATION, RELEASING and SECURITY: audited together at
  `9fa0d738`, corrected/re-verified in Revision 11; SECURITY needed no edit;
- Core API, diagnostics, FAQ, Home, installation, limitations, statistics,
  testing, version history and Time-Waster Suppression: reconciled in the
  published wiki at `f18fd82`, with two additional badge-anchor repairs.
  Content publication and all 15 emoji heading-link repairs are verified
  at `1039253`; #36 is complete. #34 remains a separate support-policy decision
  and must trigger any later documentation changes its outcome requires.

Preserve explicitly historical v1.3.0 evidence, archived plans and the exact
v1.4.0 80/643/0 and 12-check certification. Documentation-only reconciliation
does not itself add checks; the active branch now has 15 after #51 and #45.
...
Revision 8 implements this before #50, at the user's request. The chain is
12 → 13 after #51 → 15 after #45. #50's contribution is recorded only when
implemented and is never guessed in advance.

Update current v1.4.1
...
- the `release provenance strict fixtures` check runs from `vba_lint.py`, fails
  the gate on a non-zero harness result, and the active count is 13;
...
- [x] Provenance generation fails before output on every #51 invalid-input case
